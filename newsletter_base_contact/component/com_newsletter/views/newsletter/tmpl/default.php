...
<form class="cmxform" id="newsletterForm" method="post" action="">
	<fieldset>
		...
		<p>
            ...
            <input type="text" name="first_name" id="first_name" value="<?=$_SESSION['data']['first_name']; ?>" class="input-text required" minlength="2" />
        </p>
		<p>
...
<? if(!$this->newssubscription){ // Select newsletter ?>
...
if(count($this->newsletters) > 0):
    ...
    foreach ($this->newsletters as $newsletter):
        $newsletter = $newsletter->scalarval();
        ?>
            <input type="hidden" id="newsletter<?=$i;?>" name="cid[]" value="<?=$newsletter["id"]->scalarval();?>" />
		</p>
        <?
        $i++;
    endforeach;
...
<? } ?>
	...
	<fieldset id="conditions">
            <p><input type="checkbox" id="conditions" name="conditions[]" value="1" class="required"/>
            <label for="conditions"><a href="<?=$this->newsurlconditions;?>"><?=JText::_( 'Yes, accept conditions' );?></a></label></p>
            <? if($this->newssubscription): // Not show newsletter ?>
                <p><input type="checkbox" id="allnewsletter" name="allnewsletter[]" value="1" checked/>
                <label for="allnewsletter"><?=JText::_( 'Yes, recibed all newsletters' );?></label></p>
            <? endif; ?>
	</fieldset>
